...
==Example
  ...
  # => CREATE TABLE "poems" ("id" serial primary key, "author_id" integer REFERENCES "authors")
  
  # Column name specified explicitly (if you don't want _id suffix)
  create_table :poems do |t|
    t.belongs_to :author, :column => :author, :references => :poets
  end
  # => CREATE TABLE "poems" ("id" serial primary key, "author" integer REFERENCES "poets")
  
  # Non-integer primary key (for those with legacy databases) - this only works in PostreSQL now!
  execute "CREATE TABLE authors(name VARCHAR(255) PRIMARY KEY)"
  create_table :books do |t|
    t.belongs_to :author, :column => :author_name, :type => :string
  end
  # => CREATE TABLE "books" ("id" serial primary key, "author" VARCHAR(255) REFERENCES "authors")
